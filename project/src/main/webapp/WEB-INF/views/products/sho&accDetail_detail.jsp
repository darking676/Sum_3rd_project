...
<!-- 						 <br /> -->
						 ...
						 <div class="container">
							<div class="row">
								...
								<h5 style="transform: translateX(-250px) translateY(7px);">COLOR</h5>
								<h5 style="transform: translateX(-250px) translateY(12px);">SIZE</h5>
								<!-- Single button -->
								<div style="transform: translateX(-250px);" class="btn-group">

										<input type="hidden" name="productNum" value="${thisProductsVo.productNum}">
										<select name="quantity" style="transform: translateX(60px) translateY(-83px);">
											<c:forEach begin="1" end="10" var="i">
												<option value="${i }">${i }</option>
											</c:forEach>
										</select>
										<br/>
										<select name="colors" style="transform: translateX(60px) translateY(-76px);">
										...
										<br/>
										<select name="sizename" style="transform: translateX(60px) translateY(-70px);">
								...
								<button class="btn btn-info btn-sm active"
										style="width: 185px; transform: translateX(-357px)"
										type="submit" onclick="location.href='/shop01/login2/'">장바구니</button>
									<button class="btn btn-info btn-sm active"
										style="width: 185px; transform: translateX(-357px)"
										type="submit" onclick="location.href='/shop01/login2/'">결제하기</button>
									<br clear=left>
...
<!-- 									style="margin-left: 0px" class="btn btn-info btn-sm active" /> -->
		...
		<div id="form-group" style="display: none;">
			...
			<form id="modifyForm" action="/shop01/shoeacc_edit/" method="get">
				<p>
			...
			<form id="deleteForm" action="/shop01/shoeacc_delete/" method="get">
				<p>
					<input type="hidden" name="productNum" value="${param.productNum }" />
					<input type="hidden" name="procd" value="${param.procd }" /> <input
						type="hidden" name="curPage" value="${param.curPage }" /> <input
